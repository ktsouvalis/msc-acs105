 ...
 * 
 * This robot navigates around building debris using the right-hand rule,
 * detects fire/gas sources, and transmits telemetry data.
 * 
 * Hardware:
 * - Arduino Mega 2560
...
// Timing
unsigned long lastGPSTime = 0;    // Last GPS sampling time
unsigned long lastTelemetryTime = 0; // Last telemetry transmission time
...
void setup() {
  ...
  while (!missionStarted) {
    ...
    // If no GPS data received for 5 seconds, continue anyway
    if (millis() > 5000 && !missionStarted) {
  ...
  // Initial MQ-2 sensor warmup
  Serial.println("Warming up MQ-2 sensor...");
  delay(20000); // 20 seconds warmup for MQ-2 sensor
...
void loop() {
  ...
  // GPS sampling and telemetry (1Hz)
  ...
  if (currentTime - lastGPSTime >= 1000) {
    ...
    // Print mission time
    unsigned long missionTime = (currentTime - startTime) / 1000; // seconds
