...
#define MAX_RETURN_WAYPOINTS 1000
Waypoint returnPath[MAX_RETURN_WAYPOINTS];
int returnSize = 0;
int returnIdx = -1;

void loadWaypointsFromSD() {
  File f = SD.open("route.csv");
  if (!f) {
    Serial.println("Failed to open route.csv for reading");
    return;
  }
  returnSize = 0;
  ...
  while (f.available()) {
    ...
    if (comma > 0 && returnSize < MAX_RETURN_WAYPOINTS) {
      String latStr = line.substring(0, comma);
      String lonStr = line.substring(comma + 1);
      returnPath[returnSize].lat = latStr.toFloat();
      returnPath[returnSize].lon = lonStr.toFloat();
      returnSize++;
    }
  }
  f.close();
  returnIdx = returnSize - 1;
  Serial.print("Loaded waypoints: ");
  Serial.println(returnSize);
}
...
void saveCurrentWaypoint() {
  ...
  if (returnSize == 0 || gpsDistance(lastLat, lastLon, lat, lon) > WAYPOINT_MIN_DIST) {
...
void loop() {
  ...
  if (returning && returnIdx >= 0 && gps.location.isValid()) {
    double lat = gps.location.lat();
    double lon = gps.location.lng();
    float dist = gpsDistance(lat, lon, returnPath[returnIdx].lat, returnPath[returnIdx].lon);
    float bearing = gpsBearing(lat, lon, returnPath[returnIdx].lat, returnPath[returnIdx].lon);
    steerToBearing(bearing);
    moveForward(NORMAL_SPEED);
    if (dist < WAYPOINT_MIN_DIST) returnIdx--;
  } else if (!returning) {
